...
test('every module has goals, observation prompts, and definitions; 05–08 add a linear-algebra primer',()=>{
 const {SpectralCourse:c}=course(),tex=(value,display)=>`<math data-display="${display}">${value}</math>`;
 for(const lang of ['zh','en'])for(const module of c.modules){const html=c.guide(module,lang,tex);assert.match(html,/module-guide/);assert.match(html,/module-glossary/);assert.match(html,/<dt>/);assert.doesNotMatch(html,/Assumed background|預設先備知識/);if(c.modules.indexOf(module)>=5){assert.match(html,/module-primer/);assert.doesNotMatch(html,/<details class="module-primer" open/);}else assert.doesNotMatch(html,/module-primer/);}
 assert.match(c.guide('smoothness','zh',tex),/Dirichlet 能量/);assert.match(c.guide('variational','zh',tex),/有約束最佳化/);assert.match(c.guide('convolution','zh',tex),/07 如何走到 08/);
});
test('08 ends with a visible recap of 01–08 and the spectral-to-local chain',()=>{
 const {SpectralCourse:c}=course(),html=c.summary('convolution','zh',value=>`<math>${value}</math>`);assert.match(html,/實驗室總結/);for(let i=1;i<=8;i++)assert.match(html,new RegExp(`>${String(i).padStart(2,'0')}<`));assert.match(html,/local message passing/);assert.equal(c.summary('fourier','zh',String),'');
});
...
test('teaching sliders, valid descent state, 07 tab, and08 controls persist independently from the shared graph',()=>{
 const s=Session.create(M),graph=structuredClone(s.graph);Object.assign(s.variationalLab,{smoothValues:{path5:[1,1.1,1.2,1.3,1.4]},descentValues:[-.5,.5,-.5,.5],descentHistory:[1,.8,.5],descentIteration:2,descentGraphKey:'teaching-graph'});s.spectralSignal.view='filtering';s.convolutionLab={alphaFraction:.7,steps:3,selected:1};
